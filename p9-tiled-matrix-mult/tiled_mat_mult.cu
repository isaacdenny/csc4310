...
#define MAX_TILE_WIDTH 32 // because __shared__ variables are declared static
...
int tiledMatrixMult(float *M, float *N, float **P, int dim, int tile_width)
{
  ...
  matrixMulKernel<<<blocks, threadsPerBlock>>>(M_dev, N_dev, P_dev, dim,
                                               tile_width);
...
__global__ void matrixMulKernel(float *M, float *N, float *P, int Width,
                                int tile_width)
{
  // declared static so must use max for compiler
